...
const KEYWORDS = new Set([
  "ADD","ALL","ALTER","ALWAYS","AND","AS","ASC","BEGIN","BETWEEN","BY","CASCADE",
  "CASE","CHECK","COLUMN","COMMIT","CONSTRAINT","CONCURRENTLY","COUNT","CREATE",
  "CROSS","DEFAULT","DELETE","DESC","DISTINCT","DROP","ELSE","END","EXCEPT","EXISTS",
  "FALSE","FETCH","FOR","FOREIGN","FROM","FULL","GENERATED","GRANT","GROUP","HAVING",
  "IDENTITY","IF","IN","INDEX","INNER","INSERT","INTERSECT","INTO","IS","JOIN","KEY",
  "LEFT","LIMIT","NATURAL","NOT","NULL","OFFSET","ON","OR","ORDER","OUTER","PRIMARY",
  "PRECISION","PUBLIC","REFERENCES","RESTRICT","RETURNING","REVOKE","RIGHT","ROLLBACK",
  "SELECT","SET","TABLE","THEN","TO","TRUE","TRUNCATE","UNION","UNIQUE","UPDATE",
  "USING","VALUES","WHEN","WHERE","WITH","RETURNING",
]);

const TYPES = new Set([
  "BIGINT","BIGSERIAL","BIT","BOOL","BOOLEAN","BPCHAR","BYTEA","CHAR","DATE","DECIMAL",
  "DOUBLE","INT","INTEGER","INTERVAL","JSON","JSONB","MONEY","NUMERIC","REAL","SERIAL",
  "SMALLINT","SMALLSERIAL","TEXT","TIME","TIMESTAMP","TIMESTAMPTZ","UUID","VARCHAR",
]);
...
export function highlightSql(input: string): string {
  ...
  while ((m = TOKEN_RE.exec(input)) !== null) {
    ...
    if (m[1] || m[2]) {
      ...
      else if (TYPES.has(up)) out += `<span class="sql-type">${esc(tok)}</span>`;
